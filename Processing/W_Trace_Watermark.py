...
def watermark_embedding_and_extraction_test(data, strength):
    ...
    watermarked_data, watermark = run_watermark(data, strength, 16)
    extracted_watermark = unrun_watermark(watermarked_data, data, strength, 16)
    count = 0
    for i in range(len(watermark)):
        if watermark[i] == round(extracted_watermark[i]):
            count = count + 1
    print("Length of Watermark: " + str(len(watermark)))
    print("COUNT: " + str(count))

#suspicious2()
#data = csvu.extract_data("ProcessedDatasets/CLEAN/RandomSaccades/S_1011_S2_RAN.csv")
#watermark_embedding_and_extraction_test(data, 10)
#watermark_embedding_and_extraction_test()
